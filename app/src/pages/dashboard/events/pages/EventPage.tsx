...
const EventPage = ({ events, participationRequests, onRequestParticipation, onSuggestEvent, onDetails }: Props) => {
  ...
  const handleNavigateToDetails = (eventId: number) => {
    if (onDetails) {
      onDetails(eventId);
    }
  };
  ...
  return (
    <div className="event-page">
        ...
        <div className="hero-metrics">
          <div className="metric-card group">
            <div className="flex items-center gap-3">
              <div className="p-2 rounded-lg bg-lime-400/10 group-hover:bg-lime-400/20 transition-colors">
                <Activity className="w-5 h-5 text-lime-400" />
              </div>
              <div>
                <p className="metric-label">Live Opportunities</p>
                <p className="metric-value text-white">{liveEvents.length}</p>
              </div>
            </div>
          </div>
          <div className="metric-card group">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="metric-value text-white">{deadlineNearbyEvents.length}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="search-row">
        <label className="search-input-shell relative">
        ...
        <button className="suggest-btn flex items-center gap-2" onClick={() => setShowSuggestModal(true)}>
