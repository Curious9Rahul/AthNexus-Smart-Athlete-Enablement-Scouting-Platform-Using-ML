...
import { Mail, Search, Send, X, CheckCircle, Users, Trophy, TrendingUp, Eye } from 'lucide-react';
...
function matchAthletes(athletes: Athlete[], tournament: TournamentEmail): Athlete[] {
    return athletes.filter((athlete) => {
        ...
        // Basic matching on sport and level
...
export default function EmailNotificationsPage() {
    ...
    const handleSendCustomEmail = async () => {
        ...
        if (result.success) {
            ...
            toast.error('✗ Failed to send — check email address and try again', {
                description: result.error
            });
            setFailedEmails((n: number) => n + 1);
        }
    ...
    const EmailPreview = ({ athlete, tournament }: { athlete: Athlete; tournament: TournamentEmail }) => {
        // Build HTML same as backend for preview
    ...
    return (
        <div className="space-y-6">
            ...
            <div>
                ...
                <p className="text-gray-400">Send real tournament alerts to matched athletes</p>
            </div>

            {/* Stats */}
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <StatCard icon={<Users className="w-5 h-5" />} label="Total Athletes" value={athletes.length} />
                ...
                <StatCard icon={<Mail className="w-5 h-5 text-orange-400" />} label="Emails Sent" value={emailsSent} accent="bg-orange-500/20" />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                ...
                <div className="lg:col-span-3 space-y-6">
                    {!selectedTournament ? (
                        <div className="h-[400px] flex flex-col items-center justify-center border border-dashed border-white/10 rounded-2xl text-gray-500">
                        ...
                        <>
                            ...
                            <div className="bg-[#1e293b] rounded-2xl p-6 border border-white/10 shadow-xl">
                                <h3 className="text-xl font-bold text-white mb-6">Send to Any Email Address</h3>
                                ...
                                <div className="flex items-center gap-4">
                                    <div className="flex-1 px-4 py-3 bg-black/20 border border-white/5 rounded-xl text-gray-500 text-sm">
                                        <span className="text-gray-600 mr-2 font-medium">Tournament:</span>
                                        <span className="text-gray-300">{selectedTournament.tournamentName}</span>
                                    </div>
                                    <button 
                                        onClick={handleSendCustomEmail}
                                        disabled={isSendingCustom || !recipientEmail}
                                        className="h-[46px] bg-lime-400 hover:bg-lime-300 text-[#0f172a] font-bold px-6 rounded-xl flex items-center gap-2 transition-all disabled:opacity-50 shadow-lg shadow-lime-400/10"
                                    >
                                        {isSendingCustom ? <Spinner className="w-5 h-5 leading-none" /> : "Send Tournament Alert →"}
                                    </button>
                                </div>
                                {emailError && (
                            ...
                            <div className="bg-[#1e293b] rounded-2xl p-6 border border-white/10">
                                <div className="flex items-center justify-between mb-6">
                                    <h3 className="text-xl font-bold text-white">Matched Athletes ({matchedAthletes.length})</h3>
                                    <button 
                            ...
                            {previewAthlete && (
                                <div className="bg-[#1e293b] rounded-2xl border border-lime-400/30 overflow-hidden shadow-2xl animate-in fade-in slide-in-from-bottom-4">
                                    <div className="bg-lime-400/10 p-4 border-b border-white/10 flex items-center justify-between">
                                        <div className="flex items-center gap-2">
                                            <Mail className="w-4 h-4 text-lime-400" />
                                            <span className="text-white font-semibold">Email Preview — {previewAthlete.name}</span>
                                        </div>
                                        <button onClick={() => setPreviewAthlete(null)} className="text-gray-400 hover:text-white">
                                            <X className="w-5 h-5" />
                                        </button>
                                    </div>
                                    <div className="p-6 max-h-[500px] overflow-y-auto bg-gray-900">
                                    ...
                                    <div className="p-4 bg-white/5 border-t border-white/10 flex justify-end">
                                        <button 
                                            ...
                                            disabled={isSending}
                                            className="bg-lime-400 hover:bg-lime-300 text-[#0f172a] font-bold px-6 py-2 rounded-lg flex items-center gap-2"
                                        >
                                            <Send className="w-4 h-4" />
                                            Send This Email
                                        </button>
